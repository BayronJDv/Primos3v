...
int es_primo(int num) {if (num <= 1) return 0;
   if (num == 2) return 1;
   if (num % 2 == 0) return 0;
...
void* verificar_primo(void* arg) {
   ...
   total_primos += counter;
   printf("Hilo %d Encontro %d  numeros primos\n", rango->id, counter);  
   return NULL;
}
...
int main() {
   ...
   // Crear hilos
   for (int i = 0; i < num_hilos; i++) {
   	if (pthread_create(&hilos[i], NULL, verificar_primo, &rangos[i])
!= 0) {
